...
// 현실의 길이를 그대로 기다리게 하지는 않되, 행동별 상대적인 길이와 매번 다른 리듬을 살린다.
const ACTION_DURATION_RANGE: Record<HamsterAction, { minMs: number; maxMs: number }> = {
    FEED: { minMs: 5000, maxMs: 8000 },
    WATER: { minMs: 3000, maxMs: 5000 },
    PET: { minMs: 2000, maxMs: 3000 },
    WASH: { minMs: 4000, maxMs: 7000 },
    SLEEP: { minMs: 20000, maxMs: 40000 },
};
const MANUAL_WHEEL_DURATION_RANGE = { minMs: 8000, maxMs: 15000 };
// 쳇바퀴의 위쪽 테두리와 말풍선이 겹치지 않도록 중심 좌표보다 충분히 위에 표시한다.
const WHEEL_BUBBLE_OFFSET_Y = 0.23;
...
// 먹이통을 기준으로 햄스터가 밥을 먹는 위치.
const FEED_OFFSET_X = -0.005;
const FEED_UPPER_OFFSET_Y = -0.21;
const FEED_MIN_X = 0.12;
const FEED_MAX_X = 0.88;

// 가구를 보유하고 있을 때 햄스터가 스스로 하는 행동. 위에서부터 차례로 확률 구간을 차지한다.
// 확률을 낮게 둬서 "걷다가 가끔 뭘 한다"가 되게 한다.
...
const IDLE_ACTIVITIES: Array<{
...
}> = [
    {
        ...
        maxDurationMs: 8000,
        chance: 0.04,
        offsetX: FEED_OFFSET_X,
        offsetY: FEED_UPPER_OFFSET_Y,
    },
    {
        itemId: "WATER_BOTTLE",
        behavior: "DRINK",
    ...
    { itemId: "SAND_BATH", behavior: "WASH", minDurationMs: 6000, maxDurationMs: 10000, chance: 0.05, offsetY: -0.19 },
    { itemId: "SNACK_DISH", behavior: "CHEEK", minDurationMs: 4000, maxDurationMs: 7000, chance: 0.05, offsetY: 0.02 },
    { itemId: "LOOKOUT", behavior: "LOOK", minDurationMs: 5000, maxDurationMs: 8000, chance: 0.05, offsetY: 0.04 },
];
...
// 가까운 가구를 사용할 때는 걷기 애니메이션 한 주기를 채우지 않고 도착 즉시 행동으로 전환한다.
const MIN_ACTION_WALK_MS = 180;
...
function randomDurationMs({ minMs, maxMs }: { minMs: number; maxMs: number }) {
    return Math.round(minMs + Math.random() * (maxMs - minMs));
}

function steppedScale(value: number) {
    return Number(clamp(value, DISPLAY_SCALE_MIN, DISPLAY_SCALE_MAX).toFixed(2));
}

function walkDurationMs(from: StagePoint, to: StagePoint, minDurationMs = MIN_WALK_MS) {
    const distance = Math.hypot(to.x - from.x, (to.y - from.y) * STAGE_Y_WEIGHT);
    return clamp(Math.round(distance / WALK_SPEED_PER_MS), minDurationMs, MAX_WALK_MS);
}
...
export default function CagePage() {
    ...
    const [behavior, setBehavior] = useState<HamsterBehavior>(() => (isSleeping ? "SLEEP" : "IDLE"));
    ...
    const [hamsterSelected, setHamsterSelected] = useState(false);
    const [scalePreview, setScalePreview] = useState<number | null>(null);
    const [flipPreview, setFlipPreview] = useState<boolean | null>(null);
    ...
    const cageLayoutKey = cageItems.map((item) => `${item.id}:${item.itemId}:${item.posX}:${item.posY}`).join("|");
            ...
            later(
                () => {
                    ...
                    for (const activity of canUseFurniture ? IDLE_ACTIVITIES : []) {
                        ...
                        const spot = {
                            x:
                                activity.itemId === "FOOD_BOWL"
                                    ? clamp(item.posX + (activity.offsetX ?? 0), FEED_MIN_X, FEED_MAX_X)
                                    : clamp(item.posX + (activity.offsetX ?? 0), 0.18, 0.82),
    ...
    async function act(action: HamsterAction, nextBehavior: HamsterBehavior, options?: { silent?: boolean }) {
        setBusy(true);
        ...
        if (targetItem) {
            ...
            const target = {
                x:
                    action === "FEED"
                        ? clamp(targetItem.posX + FEED_OFFSET_X, FEED_MIN_X, FEED_MAX_X)
                        : clamp(targetItem.posX, 0.18, 0.82),
                y:
                    action === "FEED"
    ...
    async function rideWheel() {
        const wheel = cageItems.find((item) => item.itemId === "WHEEL");
        if (!wheel || busy || editing) return;
        ...
        await new Promise((resolve) => window.setTimeout(resolve, randomDurationMs(MANUAL_WHEEL_DURATION_RANGE)));
        try {
            await performIdleActivity("WHEEL");
            showToast(`${hamster!.name}이(가) 신나게 쳇바퀴를 탔어요.`);
        } catch (err) {
            showToast(err instanceof Error ? err.message : "쳇바퀴 행동을 완료하지 못했어요.");
        } finally {
            setActiveWheelId(null);
            setBehavior("IDLE");
            setBusy(false);
    ...
    async function putHamsterToSleep() {
        const house = cageItems.find((item) => item.itemId === "HOUSE");
        if (!house || busy || editing) return;

        setBusy(true);
        const from = hamsterPositionRef.current;
        const target = {
            x: clamp(house.posX + 0.03, 0.18, 0.82),
            y: clamp(house.posY + 0.16, 0.35, 0.8),
        };
        const duration = walkDurationMs(from, target, MIN_ACTION_WALK_MS);
        setFacing(target.x < from.x ? "left" : "right");
        setMoveDurationMs(duration);
        ...
        setBehavior("SLEEP");
        try {
            await performHamsterAction("SLEEP");
            showToast(`${hamster!.name}이(가) 집에서 잠들었어요.`);
        } catch (err) {
            setBehavior("IDLE");
            showToast(err instanceof Error ? err.message : "잠자기 행동을 완료하지 못했어요.");
        } finally {
            setBusy(false);
        }
    }

    function selectFurniture(itemId: string, scale: number, flipped: boolean) {
        setSelectedItemId(itemId);
        setHamsterSelected(false);
        setScalePreview(scale);
        setFlipPreview(flipped);
    }

    function selectHamster() {
        if (!hamster) return;
        setSelectedItemId(null);
        setHamsterSelected(true);
        setScalePreview(hamster.displayScale);
        setFlipPreview(null);
    }

    async function saveSelectedSettings() {
        if (scalePreview === null || (!hamsterSelected && !selectedItemId)) return;
        setSavingScale(true);
        try {
            if (hamsterSelected) {
                await resizeHamster(scalePreview);
            } else {
                const item = cageItems.find((candidate) => candidate.id === selectedItemId);
                if (!item) return;
                await moveCageItem(item.id, item.posX, item.posY, scalePreview, flipPreview ?? item.flipped);
            }
            showToast(hamsterSelected ? "햄스터 크기를 저장했어요." : "가구 크기와 방향을 저장했어요.");
        } catch (err) {
            showToast(err instanceof Error ? err.message : "설정을 저장하지 못했어요.");
        } finally {
            setSavingScale(false);
        }
    }

    async function storeSelectedFurniture() {
        if (!selectedItemId || savingScale) return;
        setSavingScale(true);
        try {
            await storeCageItem(selectedItemId);
            setSelectedItemId(null);
            setScalePreview(null);
            setFlipPreview(null);
            showToast("가구를 보관함에 넣었어요.");
        } catch (err) {
            showToast(err instanceof Error ? err.message : "가구를 보관하지 못했어요.");
        } finally {
            setSavingScale(false);
        }
    }

    async function placeStoredFurniture(itemId: (typeof ownedItemIds)[number]) {
        if (savingScale) return;
        setSavingScale(true);
        try {
            await placeCageItem(itemId);
            showToast(`${ITEM_MASTERS[itemId].name}을(를) 케이지에 배치했어요.`);
        } catch (err) {
            showToast(err instanceof Error ? err.message : "가구를 배치하지 못했어요.");
        } finally {
            setSavingScale(false);
        }
    }
    ...
    const actionGridClass = isSleeping
        ? "md:grid-cols-2"
        : optionalActionCount === 3
          ? "md:grid-cols-7"
          : optionalActionCount === 2
            ? "md:grid-cols-6"
    ...
    return (
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-3 md:p-6">
            ...
            <section className="grid gap-4 rounded-2xl bg-card p-4 shadow-sm lg:grid-cols-[220px_1fr]">
                ...
                >
                    {cageItems.filter((item) => item.itemId !== "HANDHELD_WATER_BOTTLE").map((item) => {
                        ...
                        const assetSrc =
                            item.id === activeWheelId ? WHEEL_SPIN_FRAMES[wheelFrameIndex] : itemAsset?.src;
                        const renderedScale =
                            selectedItemId === item.id && scalePreview !== null ? scalePreview : item.scale;
                        const renderedWidth = itemAsset ? itemAsset.widthRatio * stageWidth * renderedScale : undefined;
                        const renderedFlipped =
                            selectedItemId === item.id && flipPreview !== null ? flipPreview : item.flipped;
                        return (
                            <button
                                type="button"
                                key={item.id}
                                onClick={(event) => {
                                    event.stopPropagation();
                                    if (editing) selectFurniture(item.id, item.scale, item.flipped);
                                }}
                                onPointerDown={(event) => {
                                    if (!editing) return;
                                    event.preventDefault();
                                    selectFurniture(item.id, item.scale, item.flipped);
                                    setDraggingItemId(item.id);
                                    setPreviewPosition({ x: item.posX, y: item.posY });
                                }}
                                title={ITEM_MASTERS[item.itemId].name}
                                className={`absolute -translate-x-1/2 -translate-y-1/2 leading-none drop-shadow ${selectedItemId === item.id ? "rounded-xl ring-4 ring-accent-pink" : ""}`}
                                style={{
                                    ...
                                    touchAction: "none",
                                    // 먹는 동안에는 먹이통이 햄스터 앞에 보여 몸이 그릇 뒤에 자연스럽게 겹친다.
                                    zIndex: item.itemId === "FOOD_BOWL" && behavior === "EAT" ? 20 : 1,
                                }}
                            >
                                {itemAsset && assetSrc ? (
                                    <img
                                        ...
                                        style={{
                                            ...
                                            transform: renderedFlipped ? "scaleX(-1)" : undefined,
                                        }}
                                    />
                                ) : (
                    ...
                    <button
                        ...
                        className={`absolute -translate-x-1/2 -translate-y-1/2 leading-none ${hamsterSelected ? "rounded-xl ring-4 ring-accent-pink" : ""}`}
                        style={{
                    ...
                    {petEffects.map((effect) => (
                        <span
                            ...
                            style={{
                                ...
                                fontSize: `${Math.round(HAMSTER_WIDTH_RATIO * stageWidth * renderedHamsterScale * 0.24)}px`,
                            }}
                        >
                            💗
                        </span>
                    ...
                    {activeWheelId && (
                        <span
                            className="absolute z-30 -translate-x-1/2 rounded-full bg-card/90 px-3 py-1 text-xs font-semibold"
                            style={{
                                left: `${(activeWheel?.posX ?? 0.5) * 100}%`,
                                top: `${Math.max(0.05, (activeWheel?.posY ?? 0.8) - WHEEL_BUBBLE_OFFSET_Y) * 100}%`,
                            }}
                        >
                            쳇바퀴 타는 중
                        </span>
                    ...
                    {editing ? (
                        <p className="absolute inset-x-3 top-3 rounded-lg bg-card/90 p-2 text-center text-sm">
                            {hamsterSelected
                                ? "아래에서 햄스터 크기를 조절해 주세요."
                                : selectedItemId
                                  ? "가구를 끌어 옮기거나 아래에서 크기를 조절해 주세요."
                                  : "가구 또는 햄스터를 선택해 주세요."}
                        </p>
                    ) : isSleeping ? (
                        <p className="absolute inset-x-3 top-3 rounded-lg bg-card/90 p-2 text-center text-sm">
            ...
            {editing && (hamsterSelected || selectedItem) && scalePreview !== null && (
                <section className="rounded-2xl bg-card p-4 shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                            <strong>{hamsterSelected ? hamster.name : ITEM_MASTERS[selectedItem!.itemId].name}</strong>
                            <p className="text-sm text-brown/60">크기 {Math.round(scalePreview * 100)}%</p>
                        </div>
                        <button
                            type="button"
                            className="rounded-lg border-2 border-brown/30 px-3 py-1 text-sm"
                            onClick={() => setScalePreview(DEFAULT_DISPLAY_SCALE)}
                        >
                            기본 크기
                        </button>
                    </div>
                    {!hamsterSelected && (
                        <button
                            type="button"
                            aria-pressed={flipPreview ?? selectedItem?.flipped ?? false}
                            className="mt-3 rounded-lg border-2 border-brown/30 px-3 py-2 text-sm aria-pressed:bg-accent-yellow/40"
                            onClick={() => setFlipPreview(!(flipPreview ?? selectedItem?.flipped ?? false))}
                        >
                            ↔ 좌우 반전 {(flipPreview ?? selectedItem?.flipped ?? false) ? "켜짐" : "꺼짐"}
                        </button>
                    )}
                    <div className="mt-3 grid grid-cols-[40px_minmax(0,1fr)_40px] items-center gap-3 md:grid-cols-[40px_minmax(0,1fr)_40px_auto]">
                        <button
                            type="button"
                            aria-label="크기 줄이기"
                            className="h-10 w-10 rounded-lg bg-brown text-xl text-card"
                            onClick={() => setScalePreview(steppedScale(scalePreview - DISPLAY_SCALE_STEP))}
                        >
                            −
                        </button>
                        <input
                            aria-label="크기 조절"
                            className="min-w-0 flex-1 accent-brown"
                            type="range"
                            min={DISPLAY_SCALE_MIN}
                            max={DISPLAY_SCALE_MAX}
                            step={DISPLAY_SCALE_STEP}
                            value={scalePreview}
                            onChange={(event) => setScalePreview(Number(event.currentTarget.value))}
                        />
                        <button
                            type="button"
                            aria-label="크기 늘리기"
                    ...
                    {selectedItem && (
                        <PixelButton
                            ...
                            disabled={savingScale}
                            onClick={storeSelectedFurniture}
                        >
                            보관함에 넣기
                        </PixelButton>
                    )}
            ...
            {editing && storedItemIds.length > 0 && (
                <section className="rounded-2xl bg-card p-4 shadow-sm">
                    <h2 className="font-bold text-brown">보관 중인 가구</h2>
                    <div className="mt-3 flex flex-wrap gap-2">
                        {storedItemIds.map((itemId) => (
                            <PixelButton
                                key={itemId}
                                variant="secondary"
                                disabled={savingScale}
                                onClick={() => placeStoredFurniture(itemId)}
                            >
                                {ITEM_MASTERS[itemId].name} 배치하기
                            </PixelButton>
                        ))}
                    </div>
                </section>
            ...
            <div className={`grid grid-cols-2 gap-2 ${actionGridClass}`}>
                ...
                {isSleeping ? (
                    ...
                    availableActions.map((action) => (
                        <PixelButton
                            ...
                            onClick={() => act(action.id, action.behavior)}
                        >
                            {action.label}
                        </PixelButton>
                    ))
                )}
                {ownedWheel && !isSleeping && (
                    <PixelButton disabled={busy || editing} onClick={rideWheel}>
                        쳇바퀴 타기
                    </PixelButton>
                )}
                {ownedHouse && !isSleeping && (
                    <PixelButton disabled={busy || editing} onClick={putHamsterToSleep}>
                        잠자기
                    </PixelButton>
                )}
                {ownedSandBath && !isSleeping && (
                    <PixelButton disabled={busy || editing} onClick={() => act("WASH", "WASH")}>
                        모래 목욕
                    </PixelButton>
                )}
                <PixelButton
            ...
            <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title={`${hamster.name} 정보`}>
                <div className="space-y-2 text-sm">
                    ...
                    <PixelButton className="mt-4 w-full" onClick={() => setDetailOpen(false)}>
                        닫기
                    </PixelButton>
                </div>
            </Modal>
