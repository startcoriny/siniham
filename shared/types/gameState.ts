...
import type { CropId, GardenPlot } from "./garden";
...
export type GameStatePlot = GardenPlot;

// 자리를 비운 사이 정원에 일어난 변화. 확인(ack)하기 전까지 유지되고, 변화가 없으면 null이다.
export interface GardenSummary {
  grownCount: number;
  weedCount: number;
  grownByCrop?: Partial<Record<CropId, number>>;
  since: string | null;
}
...
export interface GameStateResponse {
  ...
  produceInventory: Record<CropId, number>;
